...
Scalar ComputeQuadradicSubdivisions(Scalar scale_factor,
  ...
  return std::sqrt(k * length(p0 - cp * 2 + p1));
}
...
Scalar ComputeConicSubdivisions(Scalar scale_factor,
  ...
  const Scalar max_len =
      std::sqrt(std::fmax(p0.Dot(p0), std::fmax(p1.Dot(p1), p2.Dot(p2))));
  ...
  const Scalar rp_minus_1 = std::fmax(0.0f, max_len * k - 1);
  const Scalar numer = std::sqrt(dp.Dot(dp)) * k + rp_minus_1 * dw;
  const Scalar denom = 4 * std::fmin(w, 1.0f);

  // Number of segments = sqrt(numerator / denominator).
